...
int main() {
	...
	while (!cerrar) {   
		...
		if (!fuera_mainmenu && !ball1.vx && !ball1.vy) {  // Dimensionamiento de las entidades del juego
		...
		if (event.type == ALLEGRO_EVENT_TIMER) { // si el usuario no ingresa nada simplemente se redibuja el estado en el que estaba
			if (fuera_mainmenu) {
		 		...
		 		// chequea si la pelota choco contra las paredes del juego //
				...
				// chequea si la pelota choco contra el techo //
				...
				// chequea si la pelota se cae al vacio //
				...
				// variable que representa el tipo de choque entre la pelota y la plataforma
				...
				bool colisiono = false;  // flag que se pone en falso cuando la pelota choca contra la plataforma. da lugar a un choque con un ladrillo
				
				// chequea todos los bloques de la matriz y analiza si alguno fue golpeado por la pelota //
				for (int i = 0; i < ALTO && !colisiono; i++) {
					for (int j = 0; j < ANCHO && !colisiono; j++) {
				
						// variable que representa el tipo de choque entre la pelota y un ladrillo
						...
						if (collision_wb == -1) {
							...
							if (mat[i][j].estado) {  // redirecciona la pelota si el bloque sigue existiendo
				...
				// se guardan las nuevas posiciones de la pelota //
			...
			// borra la pantalla y la vuelve a dibujar ya actualizada
